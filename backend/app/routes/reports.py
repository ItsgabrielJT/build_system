...
async def report_income(
    ...
    if format == "csv":
        ...
        content = await service.income_pdf(period)
        return _pdf_response(content, f"{filename_base}.pdf")
    else:  # excel
        content = await service.income_excel(period)
...
async def report_balance(
    ...
    if format == "csv":
        ...
        content = await service.balance_pdf(period)
        return _pdf_response(content, f"{filename_base}.pdf")
    else:  # excel
        content = await service.balance_excel(period)
        return _excel_response(content, f"{filename_base}.xlsx")
